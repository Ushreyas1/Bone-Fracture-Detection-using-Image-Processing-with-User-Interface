...
def _reshape_img(arr):
    ...
    # Reshape the numpy image and return a 1-D array
...
def train_and_save(train_img_list, label_list, model_name):
    ...
    return model
##
def test_and_save(test_img_list, label_list, model_name):
    try:
        with open(model_name,"rb") as file:
            model= pickle.load(file)
    except FileNotFoundError:
...
##
    ...
    svm_model=test_and_save(test_img_list,test_label_list, "ridge_model")
